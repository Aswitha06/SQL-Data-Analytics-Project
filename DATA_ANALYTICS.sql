...
delimiter $$
create procedure GetDept_id(IN ID INT)
begin
select dept_id from employess
where dept_id=ID;
end $$
delimiter ;
call getDept_id(101);
...
delimiter ##
create procedure insert_employee(in id int,in name varchar(50))
begin
insert into employess( emp_id,employee_name) 
values(id,name);
end ##
delimiter ;
call insert_employee(101,'Arun');

-- > STORED PROCEDURE WITH IN AND OUT PARAMETERS.
delimiter @@ 
create procedure inout_parameter(inout monthly_salary int)
begin
set monthly_salary=monthly_salary+1000;
end @@
delimiter ;
set @x=500000;
call inout_parameter(@x);
select(@x);

-- > CREATE FUNCTION TO CALCULATE YEARLY SALARY.
delimiter **
create function yearly_salary(salary int)
returns int
deterministic
begin
return salary * 12;
